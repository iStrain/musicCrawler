 ...
 * Used with gratitude.
 * All rights with respect to the original example remain reserved by them.
 */
...
public class RootCrawler {
    ...
    public RootCrawler(MusicLibrary ml, String startingDir) {
	@SuppressWarnings("unused")
	final JFXPanel fxPanel = new JFXPanel(); // Needed to initialise Media toolkit
	this.ml = ml;
	this.startingDir = Paths.get(startingDir);
	this.finder = new Finder("*.mp3");
	this.numMatches = 0;
	this.numAdded = 0;
	this.done = false;

	try {
	    Files.walkFileTree(this.startingDir, this.finder);
	    while (!this.finder.getDone())
		;
